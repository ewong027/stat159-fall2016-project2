...
From the cross validation, we were able to find the best model which was `r lambda_min_pc`. When comparing coefficents between PCR and ordinary least squares, I find that they are very similar to one another, except in Limit and Rating where there is a larger difference. When looking at PCR's coefficents, I notice that PCR's coefficents were most similar to that of ridge regression and PLSR. Overall PCR's coefficents tends to be smaller than that of ordinary least squares. Finally, the MSE of PC is `r pc_mse` and is larger than that of ordinary least squares.
...
Then using the full dataset, we came up the ceofficients in the `Regression Coefficients` Table.
